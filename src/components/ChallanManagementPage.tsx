...
export function ChallanManagementPage() {
  ...
  const fetchClients = async () => {
    try {
      ...
      const transformedData = data?.map(client => ({
        ...client,
        challans: Array.isArray(client.challans) ? client.challans.length : 0,
        returns: Array.isArray(client.returns) ? client.returns.length : 0
      }));

      if (error) throw error;

      const mappedClients = transformedData?.map(client => ({
        ...client,
    ...
    try {
      ...
      const { data: udharData, error: udharError } = await supabase
        ...
        .select(`
          ...
          challan_date,
          status,
          challan_items:challan_items(plate_size, borrowed_quantity)
        `)
  ...
  return (
    <div className="space-y-4">
      {/* Search Header */}
      <div className="sticky top-0 bg-gray-50 z-50 -mx-4 px-4 py-3 border-b border-gray-200">
